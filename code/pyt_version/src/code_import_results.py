...
def import_k2_results(delineation_name, discretization_name, parameterization_name, simulation_name, simulation_abspath): 
    
    tweet(f"    Reading results from output file")
    ...
    tweet("    Importing results into table k2_results")
    sim_name = os.path.split(simulation_abspath)[1]
    results_gdb_abspath = os.path.join(simulation_abspath, f"{sim_name}_results.gdb")
    if arcpy.Exists(results_gdb_abspath):
        arcpy.management.Delete(results_gdb_abspath)
    
    result = arcpy.management.CreateFileGDB(
        out_folder_path=simulation_abspath,
        out_name=f"{sim_name}_results.gdb",
        out_version="CURRENT")
    results_gdb_abspath = result.getOutput(0)

    table_name = "k2_results"    
    temp = tempfile.NamedTemporaryFile(suffix=".csv", delete=False)
    df_results.to_csv(temp.name, index=False)
    arcpy.TableToTable_conversion(temp.name, results_gdb_abspath, table_name)
    add_field_alias(results_gdb_abspath, table_name)
...
def read_simulation_results(delineation_name, discretization_name, parameterization_name, simulation_name, simulation_abspath):
    ...
    with open(runfile_abspath, "r") as runfile:
        for line in runfile:  # this is to handle Batch runs
            ...
            df_metric_english["CreationDate"] = datetime.datetime.now()
            df_metric_english["AGWAVersionAtCreation"] = "4.0.0"
            df_metric_english["AGWAGDBVersionAtCreation"] = "4.0.0"
