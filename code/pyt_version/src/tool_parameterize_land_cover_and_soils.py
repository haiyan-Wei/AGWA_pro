...
class ParameterizeLandCoverAndSoils(object):
    ...
    def getParameterInfo(self):
        ...
        param7 = arcpy.Parameter(displayName="Soils Database",
                                 name="Soils Database",
                                 datatype="GPString",
                                 parameterType="Optional",
                                 direction="Input")

        param8 = arcpy.Parameter(displayName="Maximum Number of Soil Horizons",
                                 name="Max_horizons",
                                 datatype="GPLong",
                                 parameterType="Optional",
                                 direction="Input")
        
        param9 = arcpy.Parameter(displayName="Maximum Soil Depth (cm)",
                                 name="Max_thickness",
                                 datatype="GPDouble",
                                 parameterType="Optional",
                                 direction="Input")
        
        param10 = arcpy.Parameter(displayName="Channel Type",
                                 name="Channel_Type",
                                 datatype="GPString",
                                 parameterType="Optional",
                                 direction="Input") 

        param11 = arcpy.Parameter(displayName="Parameterization Name",
                                 name="Parameterization_Name",
                                 datatype="GPString",
                                 parameterType="Optional",
                                 direction="Input")

        param12 = arcpy.Parameter(displayName="Environment",
                                 name="Environment",
                                 datatype="GpString",
                                 parameterType="Optional",
                                 direction="Input")
        param12.filter.list = ["ArcGIS Pro", "ArcMap", "Geoprocessing Service"]
        param12.value = param12.filter.list[0]
        ...
        param15 = arcpy.Parameter(displayName="Debug messages",
                                 name="Debug",
                                 datatype="GPString",
                                 parameterType="Optional",
                                 direction="Input")
        param15.value = "default"
    ...
    def get_previous_element_soil_cover_parameterization(self, prjgdb, delineation_name, discretization_name):
    ...
    def updateParameters(self, parameters):
        ...
        pre_soil_cover_list, pre_element_list = self.get_previous_element_soil_cover_parameterization(prjgdb, delineation_name, discretization_name)
        parameters[11].filter.list = pre_element_list
        ...
        if arcpy.Exists(lookup_table):
            ...
            parameters[10].filter.list = channel_type_list
        else:
    ...
    def updateMessages(self, parameters):
        ...
        if use_previous:
            ...
            parameterization_name = parameters[11].valueAsText
        ...
        if parameters[0].value and parameters[1].value and len(pre_element_list) == 0:
            msg = (f"Element parameterization must be performed prior to land cover and soils" 
                   f"parameterization for this delineation and discretization.")
            parameters[11].setErrorMessage(msg)
        
        if use_previous:
            if parameters[0].value and parameters[1].value and parameters[11].value:
        ...
        if parameters[0].value:
            channel_type_list = parameters[10].filter.list
    ...
    def execute(self, parameters, messages):
        ...
        if use_previous_parameterization:
            ...
            soils_database = parameters[7].valueAsText
            max_horizons = int(parameters[8].valueAsText)
            max_thickness = float(parameters[9].valueAsText)
            channel_type = parameters[10].valueAsText

        parameterization_name = parameters[11].valueAsText
        environment = parameters[12].valueAsText
        workspace = parameters[13].valueAsText
        prjgdb = parameters[14].valueAsText
        debug = parameters[15].valueAsText
        ...
        if use_previous_parameterization: 
            agwa.copy_parameterization(prjgdb, workspace, delineation, discretization, previous_parameterization)
        else:
