...
def write_file(output_file, agwa_version_at_creation, agwa_gbd_version_at_creation, delineation, discretization,
    ...
    file_info = (f"! File Info\n"
                 ...
                 f"!  Number of Planes:                       {number_of_hillslopes}\n"
    ...
    for squc in range(1, number_of_channels + 1):
        ...
        # get lateral ID
        lat_id = f"{channel_id - 2} {channel_id - 1}"

        # get upland ID
        if channel_id - 3 in df_hillslopes .HillslopeID.values:
            up_hillslope = [channel_id - 3, channel_id - 2, channel_id - 1]
            up_id = f"{channel_id - 3}"
        else:
            up_hillslope = [channel_id - 2, channel_id - 1]
            up_channel = df_contributing_channels.loc[
                df_contributing_channels.ChannelID == f"{channel_id}", "ContributingChannel"].values
            up_id = " ".join(list(map(str, up_channel)))

        # write parameters
        for hillslope_id in up_hillslope:
...
def write_hillslope(plane_id, par_name, df_hilllslopes):
    ...
    slope = hillslope_parameters.MeanSlope
    ...
    plane_info = ("BEGIN PLANE\n"
                    f"  ID = {plane_id}, PRINT = 3, FILE = planes\plane_{plane_id}.sim\n"
