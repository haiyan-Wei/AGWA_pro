...
def process_compare(simulation_path, compare_method, base_simulation, target_simulation, compare_name, workspace):
    """Compare the results of two simulations."""
    try:
        # Construct paths to the results tables
        base_results_gdb = os.path.join(simulation_path, base_simulation, f"{base_simulation}_results.gdb")
        base_results_table = os.path.join(base_results_gdb, "k2_results")
        target_results_gdb = os.path.join(simulation_path, target_simulation, f"{target_simulation}_results.gdb")
        target_results_table = os.path.join(target_results_gdb, "k2_results")

        # Extract fields and load data into DataFrames
        common_fields = set(f.name for f in arcpy.ListFields(base_results_table)).intersection(
                        f.name for f in arcpy.ListFields(target_results_table))
        abs_fields_to_compare = [field for field in common_fields if field in {
        ...
        df_base = pd.DataFrame(arcpy.da.FeatureClassToNumPyArray(base_results_table, fields_to_compare + index_fields))
        df_target = pd.DataFrame(arcpy.da.FeatureClassToNumPyArray(target_results_table, fields_to_compare + index_fields))
